function parsePokemonData(pokemonInfo) {
  ...
  let nicknameMatch = firstLineName.match(/^(.+)\s+\((.+)\)\s+\((F|M)\)$/)

  if (nicknameMatch) {
    pokemon.nickname = nicknameMatch[1].trim()
    pokemon.name = nicknameMatch[2].trim()
    if (nicknameMatch[3]) {
      pokemon.gender = nicknameMatch[3] === "F" ? "female" : "male"
    }
  } else {
    // Case: Name (Gender)
  ...
  if (pokemon.name === "Tornadus" || pokemon.name === "Thundurus" || pokemon.name === "Landorus" || pokemon.name === "Enamorus") {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (line.startsWith("Ability:")) {
      ...
      pokemon.level = Number.parseInt(line.substring("Level:".length).trim(), 10)
    } else if (line.startsWith("Shiny:")) {
      pokemon.shiny = line.substring("Shiny:".length).trim() === "Yes"
    } else if (line.startsWith("Happiness:")) {
      pokemon.happiness = Number.parseInt(line.substring("Happiness:".length).trim(), 10)
